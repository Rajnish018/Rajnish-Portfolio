      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
        ...
        <section className="flex-1 w-full">
          ...
          <div className="space-y-16 relative">
            <div className="absolute left-0 top-2 bottom-0 w-px bg-gradient-to-b from-accent via-white/20 to-transparent" />
            {experience.length === 0 ? (
              ...
              experience.map((exp, i) => (
                <motion.div key={exp._id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: i * 0.1 }} viewport={{ once: true }} className="relative pl-10 group">
                  <div className="absolute top-2 left-[-5px] w-[11px] h-[11px] rounded-full bg-background border-2 border-accent shadow-[0_0_10px_rgba(124,58,237,0.5)] group-hover:bg-accent transition-colors duration-300" />
                ...
                ?.items.map((skill, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className="glass-card p-6 hover:border-accent/50 transition-colors group flex flex-col justify-between min-h-[160px]"
                    >
                      <div>
