...
export const Projects: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Try to use cached projects so loader shows only on first visit
    try {
      ...
      if (loadedFlag === 'true') {
        // already loaded in this session — don't show loader, fetch updates in background
        setLoading(false);
        fetchProjects(true);
      } else {
        // first time: show loader while fetching
        fetchProjects(false);
      }
    } catch (e) {
  ...
  if (loading) {
    ...
    >
          <div className="min-h-screen pt-24 md:pt-32 px-4 md:px-6 pb-20 max-w-7xl mx-auto overflow-x-hidden">
              ...
              <div className="flex items-center gap-3 overflow-x-auto pb-4 md:pb-0 no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`flex items-center px-5 py-2.5 rounded-full text-[9px] font-bold uppercase tracking-widest transition-all whitespace-nowrap shrink-0 ${filter === cat.name
            ...
            {/* Loader placed only in the projects grid area (below filter) */}
